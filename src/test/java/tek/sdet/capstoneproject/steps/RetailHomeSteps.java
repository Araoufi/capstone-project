...
public class RetailHomeSteps extends CommonUtility {
	...
	public void userSelectQuantity(String add) {
		sendText(factory.homePage().Quantity, add);
		logger.info("user selected quantity to '2'");
		slowDown();
	}
	...
	public void TheCartIconQuantityShouldChangeTo2(String two) {
		Assert.assertTrue(isElementDisplayed(factory.homePage().cartBtn));
	...
	@And("User click on proceed to Checkout button")
	public void userClickOnProceedCheckOut() {
		click(factory.homePage().proceedBtn);
		logger.info("user clicked on Proceed to Checkout");
		slowDown();
	}
